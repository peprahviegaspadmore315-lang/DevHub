...
import { FaceLockScanner, type FaceLockScannerState } from './FaceLockScanner';
import { faceLockService } from '@/services/faceLockService';

export interface FaceLockModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const FaceLockModal: React.FC<FaceLockModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  ...
  const [scannerState, setScannerState] = useState<FaceLockScannerState>('idle');
  const [message, setMessage] = useState('');
  const [mode, setMode] = useState<'verify' | 'register' | 'register-choice'>(initialMode === 'verify' && faceLockService.isRegistered() ? 'verify' : initialMode === 'register' ? 'register' : 'register-choice');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialMode === 'verify') {
        if (faceLockService.isRegistered()) {
          setMode('verify');
          setMessage('Tap start, then approve the face prompt from your device.');
        } else {
        ...
        setMode('register');
        setMessage('Tap start, then register this device Face ID for DevHub.');
      }
    }
  }, [isOpen, initialMode]);
  ...
  const handleRegisterStart = async () => {
    try {
      if (!accountEmail?.trim()) {
        ...
        setMessage('Enter your email first so DevHub can link this face profile to your account.');
        return;
      }

      setIsProcessing(true);
      setScannerState('scanning');
      setMessage('Approve the Face ID prompt from your device...');

      const faceData = await faceLockService.registerFace(accountEmail);

      setScannerState('success');
      setMessage('Face registered successfully!');

      setTimeout(() => {
        onSuccess(true, faceData.deviceId, faceData.ownerEmail);
      }, 900);
    } catch (error) {
      setScannerState('error');
  ...
  const handleVerifyStart = async () => {
    try {
      ...
      const result = await faceLockService.verifyFace(
        (msg) => setMessage(msg),
        () => setScannerState('scanning')
      );
  ...
  const handleRegisterChoice = (registerNow: boolean) => {
    if (registerNow) {
      setMode('register');
      setScannerState('idle');
      setMessage('Ready to register your face');
      setIsProcessing(false);
    } else {
      // Skip registration
      onSuccess(false);
            ...
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-purple-500/30 rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center justify-between p-6 border-b border-purple-500/20">
                <h2 className="text-xl font-bold text-white">
                  {mode === 'register-choice' && 'Face Lock Authentication'}
                  {mode === 'register' && 'Register Your Face'}
                  {mode === 'verify' && 'Verify Your Face'}
                </h2>
                <button
                  onClick={onClose}
                  ...
                  >
                    <div className="space-y-3">
                      <p className="text-gray-300">
                        You haven't registered your face yet. Would you like to set it up now for faster authentication?
                      </p>
                      <p className="text-xs text-gray-500">
                        DevHub will ask this browser to use your device Face ID or platform biometric prompt before sign-in is allowed.
                      </p>
                    </div>
